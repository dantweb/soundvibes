...
class WhisperEngine:
    ...
    def __init__(
        ...
        compute_type: str = _DEFAULTS.compute_type,
        announce=print,
    ) -> None:
    ...
    def transcribe(self, audio: np.ndarray, language: str | None, beam_size: int) -> EngineResult:
        ...
        return EngineResult(
            segments=[
                Segment(segment.text, segment.no_speech_prob, segment.avg_logprob)
...
class TextAssembler:
    ...
    def __init__(
        self,
        hallucinations: HallucinationFilter | None = None,
        max_no_speech_probability: float = _DEFAULTS.max_no_speech_probability,
        min_average_logprob: float = _DEFAULTS.min_average_logprob,
    ) -> None:
    ...
    def _is_plausible(self, segment: Segment) -> bool:
        return (
            segment.no_speech_probability <= self._max_no_speech_probability
...
class TranscriptionService:
    ...
    def __init__(
        ...
        assembler: TextAssembler | None = None,
    ) -> None:
        self._engine = engine
    ...
    def transcribe(self, utterance: Utterance) -> TranscriptLine | None:
    ...
    def _pinned_language(self) -> str | None:
        """With one allowed language there is nothing to detect."""
        return self._languages[0] if len(self._languages) == 1 else None

    def _constrain_language(self, utterance: Utterance, result: EngineResult) -> EngineResult:
        ...
        retried = self._engine.transcribe(
            utterance.audio, language=language, beam_size=self._beam_size
        )
        return EngineResult(
    ...
    def _best_allowed(self, probabilities) -> tuple[str, float] | None:
        allowed = [
            (code, probability)
            for code, probability in (probabilities or [])
