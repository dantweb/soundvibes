...
class FakeEngine:
    ...
    def transcribe(self, audio, language, beam_size):
        self.calls.append({"language": language, "beam_size": beam_size, "samples": len(audio)})
        if self.results:
            return self.results.pop(0)
        return EngineResult(
            segments=[], language="en", language_probability=0.0, all_language_probabilities=[]
        )
...
def make_result(
    ...
    return EngineResult(
        segments=[
            Segment(text=text, no_speech_probability=no_speech, average_logprob=average_logprob)
        ],
        language=language,
